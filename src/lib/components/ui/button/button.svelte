	...
	export const buttonVariants = tv({
		...
		variants: {
			variant: {
				...
				glow: 'bg-primary text-primary-foreground shadow-lg shadow-primary/50 hover:bg-primary/90'
			},
			size: {
				default: 'h-9 px-4 py-2 has-[>svg]:px-3',
				sm: 'h-8 gap-1.5 rounded-md px-3 has-[>svg]:px-2.5',
				lg: 'h-10 rounded-xl px-6 has-[>svg]:px-4',
				ex: 'h-12 rounded-full text-md px-8 has-[>svg]:px-5',
				icon: 'size-9'
			}
		},
		defaultVariants: {
...
<Motion whileTap={{ scale: 0.95 }} transition={{duration: 0.015, bonus: 0 }} let:motion>
	{#if href}
		<a
			bind:this={ref}
			data-slot="button"
